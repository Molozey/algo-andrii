...
def HoldingPosition(open_logic, dot_close_tuple, dot_high_tuple, dot_low_tuple, holdParams, arrow_index, time_border_counter):
    if open_logic['type_operation'] == 'BUY':
        if time_border_counter - 1 > holdParams['time_barrier_param']:
            ...
            return {'type_holding': 'stopLoss', 'close_price': dot_high_tuple[arrow_index], 'close_index': arrow_index}
        elif dot_high_tuple[arrow_index] >= open_logic['take_profit_border']:
            return {'type_holding': 'takeProfit', 'close_price': dot_high_tuple[arrow_index], 'close_index': arrow_index}
...
# inp_data.beautiful_time = pd.date_range(pd.Timestamp('2020-01-01 00:00:00'), pd.Timestamp('2020-01-01 00:00:00') + pd.Timedelta(f"{inp_data.shape[0] - 1}T"), freq='1T')
...
#     "MaxHold": [str(x)+'T' for x in np.linspace(20, 450, 6)],
#     "WindowRoll": [str(x)+'T' for x in np.linspace(20, 450, 6)],
...
#
# SHIFT = 200_000
# RESULT = Parallel(n_jobs=-1, verbose=5, prefer="threads", require='sharedmem')(delayed(_estimator)(inp_data.copy().iloc[250_000:].iloc[:SHIFT], create_grid(dict(shuffled.iloc[paramArrow])), show=False) for paramArrow in tqdm(range(shuffled[:20].shape[0])))
#
...
#
# df, preprocessed_data, optimizePar = _estimator(data_frame=inp_data.copy().loc['2020-09':], params_dict=create_grid(dict(optParams)), show=True)
# # plotter(df, preprocessed_data)
# lightPlotter(df, preprocessed_data)
...
# inp_data.index = inp_data.beautiful_time
SHIFT = 800_000
df, preprocessed_data, optimizePar,  = _estimator(data_frame=inp_data.copy().loc['2019-01-10':].iloc[:SHIFT], params_dict=create_grid(params), show=True)
print('Calculated...')
lightPlotter(df, preprocessed_data)