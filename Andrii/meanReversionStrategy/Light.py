...
def create_csv(file_name, numberST):
    ...
    params = {
        "BuyLossPercent": [round(x, 3) for x in np.linspace(.1, 2.1, 6)],
        #"SellLossPercent": [20, 30, 40],
        "BuyTakePercent": [round(x, 3) for x in np.linspace(1, 2.1, 6)],
    ...
    inSample = inp_data.copy().loc['2018-10-01': '2019-09-15']
    for paramArrow in tqdm(range(shuffled[:numberST].shape[0])):
        RESULT.append(_estimator(inSample.copy(),
