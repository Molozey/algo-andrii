...
def create_csv(file_name, numberST):
    ...
    params = {
        "BuyLossPercent": [round(x, 3) for x in np.linspace(.2, 4.8, 6)],
        #"SellLossPercent": [20, 30, 40],
        "BuyTakePercent": [round(x, 3) for x in np.linspace(1, 4.8, 6)],
        #"SellTakePercent": [20, 30, 40],
        "MaxHold": [str(int(x))+'T' for x in np.linspace(30, 950, 12)],
        "WindowRoll": [str(int(x))+'T' for x in np.linspace(30, 950, 18)],
