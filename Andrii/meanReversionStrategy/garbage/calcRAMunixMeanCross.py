...
def create_csv(file_name):
    ...
    def create_grid(pam_grid_obj):
        ...
        VR_lookBack = pam_grid_obj['VRlookback']
        VR_rolling = pam_grid_obj['VRcarette']
        ...
        PARAMS = {'Capital': CAP,
                  ...
                  'variance_ratio_lookback': VR_lookBack,
                  'variance_ration_carette': VR_rolling
                  }
    ...
    params = {
        "BuyLossPercent": np.linspace(.2, 1.8, 5),
        #"SellLossPercent": [20, 30, 40],
        "BuyTakePercent": np.linspace(1, 4.8, 5),
        #"SellTakePercent": [20, 30, 40],
        "MaxHold": [str(int(x))+'T' for x in np.linspace(100, 2450, 25)],
        "WindowRoll": [str(int(x))+'T' for x in np.linspace(100, 950, 7)],
        "Y_STD": np.linspace(100, 300, 4)
              }
    ...
    RESULT = Parallel(n_jobs=-1, verbose=5, prefer="threads", require='sharedmem', batch_size=50)(
        delayed(_estimator)(inp_data.copy().loc['2020-06-01': '2021-06-01'],
                            create_grid(dict(shuffled.iloc[paramArrow])), show=False) for paramArrow in
        tqdm(range(shuffled[:3020].shape[0])))
