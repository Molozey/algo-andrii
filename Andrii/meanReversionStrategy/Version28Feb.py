...
def vrRatio(X, ka):
    def ret(typ):
        return np.around(1 + np.diff(typ) / typ[1:], 5)

    q = int(pd.Timedelta(ka) / '1T')
    log_rets = np.log(ret(np.array(X)))
    log_close = np.log(X)
    T = len(log_rets)
    mean = np.mean(log_rets)
    sqr = np.square(log_rets - mean)
    var_1 = np.sum(sqr) / T

    rets_lag = (log_close - np.roll(log_close, q))[q:]
    m = q * (T - q + 1) * (1 - q / T)
    var_k = np.sum(np.square(rets_lag - q * mean)) / m
    return var_k /var_1

def create_csv(file_name, numberST):
        ...
        try:
            ...
            while ISX < (data_frame.shape[0] / 1.1) and (cycle_buffer != ISX):
                ...
                openLogic = OpenPosition(dot_low_tuple=dot_low_tuple, dot_high_tuple=dot_high_tuple,
                                         ...
                                         arrow_index=ISX, openParams=params_dict, correct_borders=correct_borders, opens=dot_open_tuple, recursion_limiter=0)
                while type(openLogic) != dict:
                    openLogic = OpenPosition(dot_low_tuple=dot_low_tuple, dot_high_tuple=dot_high_tuple,
                                             LowBBand_tuple=LowBBand_tuple, HighBBand_tuple=HighBBand_tuple,
                                             arrow_index=openLogic, openParams=params_dict, correct_borders=correct_borders,
    ...
    params = {
        "BuyLossPercent": [round(x, 3) for x in np.linspace(.1, 5.1, 4)],
        #"SellLossPercent": [20, 30, 40],
        "BuyTakePercent": [round(x, 3) for x in np.linspace(1, 5.1, 4)],
        #"SellTakePercent": [20, 30, 40],
        "MaxHold": [str(int(x))+'T' for x in np.linspace(40, 950, 7)],
        "WindowRoll": [str(int(x))+'T' for x in np.linspace(40, 400, 5)],
        "Y_STD":[round(x, 2) for x in np.linspace(130, 300, 4)],
        'caretteHYP': [10, 20, 40],
        'VR': [round(x, 2) for x in np.linspace(.6, 1.2, 5)]
              }
    grid = ParameterGrid(params)
    print(f"Grid len = {len(grid)}")
    # shuffled = pd.DataFrame(grid).sample(frac=1, random_state=20).reset_index(drop=True) # OLD PARAMS
    shuffled = pd.DataFrame(grid).sample(frac=1, random_state=9).reset_index(drop=True)
    ...
    for paramArrow in tqdm(range(shuffled[:numberST].shape[0])):
        RESULT.append(_estimator(inp_data.copy().loc['2018-10-01': '2019-09-15'],
                            create_grid(dict(shuffled.iloc[paramArrow])), show=False))
