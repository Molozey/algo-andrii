...
def create_csv(file_name):
    ...
    RESULT = Parallel(n_jobs=-1, verbose=10, prefer="threads", batch_size=10)(
        delayed(_estimator)(inp_data.copy().loc['2018-10-01': '2019-09-15'],
                            create_grid(dict(shuffled.iloc[paramArrow])), show=False) for paramArrow in
        tqdm(range(shuffled[:290].shape[0])))
