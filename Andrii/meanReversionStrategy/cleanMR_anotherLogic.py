...
params = {
    "BuyLossPercent": np.linspace(.1, 1.2, 4),
    #"SellLossPercent": [20, 30, 40],
    "BuyTakePercent": np.linspace(.1, 1.2, 4),
...
df, preprocessed_data, optimizePar,  = _estimator(data_frame=inp_data.copy().loc['2019-01-01': '2020-01-01'], params_dict=create_grid(dict(optParams)), show=True)
...
df, preprocessed_data, optimizePar = _estimator(data_frame=inp_data.copy().loc['2020-01-01': '2021-01-01'], params_dict=create_grid(dict(optParams)), show=True)
# plotter(df, preprocessed_data)
lightPlotter(df, preprocessed_data)
print('OUTOFSAMPLE')
print(f"Total trades:", df.shape[0])
