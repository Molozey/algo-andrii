...
def strategy_real_time_optimize(realTimeData, parameters, show=True, update=False):
    ...
    while POSITION < SL:
        ...
        # Оптимизировать 0!
        'halfToFat': np.unique([z if z != 0 else 1 for z in [float(x) for x in np.linspace(optimalParams["halfToFat"] * 0.9,
                                                                                         optimalParams["halfToFat"] * 1.1, num=3)]]),
        # Оптимизировать 0!
        'halfToLight': np.unique([z if z != 0 else 1 for z in [float(x) for x in np.linspace(optimalParams["halfToLight"] * 0.9,
                                                                                          optimalParams["halfToLight"] * 1.1, num=3)]]),
        # Оптимизировать 0!
        'halfToTime': np.unique([z if z != 0 else 1 for z in [float(x) for x in np.linspace(optimalParams["halfToTime"] * 0.9,
...
if __name__ == '__main__':
    ...
    RES, EVOLUTION = _get_the_best_strategy(mainOptimizerData=inpData.copy(), initConditions=grid_params,
                                            numberOfDotes=12, saveInfo=True)
