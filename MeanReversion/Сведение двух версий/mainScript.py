...
def open_position(position, dataFrame, params, reqCounter, preComputed):
    ...
    try:
        ...
        print('Inside')
        print(f"position={position} scan={params['scanHalfTime']}")
        print(dataFrame.close[position-params['scanHalfTime']:position])
        raise ValueError
    ...
    while POS < SL:
        ...
        while not isinstance(openPosition, dict):
            ...
            try:
                ...
                print('Outside')
                print(Collparameters)
                print(e)
                print(f"POS={POS} SL={SL}")
                print(coll_DATAFRAME.iloc[POS - leftShift: POS + rightShift + 1])
...
def _estimator(_DATAFRAME, _gridParams: dict):
    ...
    openShift = max(Estparameters['scanHalfTime'], int(Estparameters['scanHalfTime'] * Estparameters['halfToFat']))
    leftShift = max(Estparameters['scanHalfTime'], int(Estparameters['scanHalfTime'] * Estparameters['halfToFat']))
    # ИЗМЕНЕНИЕ НЕПРОВЕРЕНО: В ПРАВУЮ ГРАНИЦУ ДОБАВЛЕНА + 1
    # !!!!!!!!!!!!!!!!!
...
def strategy_real_time_optimize(realTimeData, parameters, savePath: str, show=True, update=False):
    ...
    _UPDATE_TIME //= '1T'
    _TRADE_TIME //= '1T'
    SL = int((SL // _TRADE_TIME) * _TRADE_TIME)
    ...
    while POSITION < SL:
        ...
        optimizing_grid = {
            ...
            'scanHalfTime': np.unique(
                [z if z != 0 else 1 for z in [int(x) for x in np.linspace(optimalParams["scanHalfTime"] * 0.92,
                                                                          optimalParams["scanHalfTime"] * 1.08,
                                                                          num=3)]]),
            # Оптимизировать 0!
            'halfToFat': np.unique(
                [round(z, 3) if z != 0 else 1 for z in [float(x) for x in np.linspace(optimalParams["halfToFat"] * 0.92,
                                                                                      optimalParams["halfToFat"] * 1.08,
                                                                                      num=3)]]),
            # Оптимизировать 0!
            'halfToLight': np.unique([round(z, 3) if z != 0 else 1 for z in
                                      [float(x) for x in np.linspace(optimalParams["halfToLight"] * 0.92,
                                                                     optimalParams["halfToLight"] * 1.08, num=3)]]),
            # Оптимизировать 0!
            'halfToTime': np.unique([round(z, 3) if z != 0 else 1 for z in
                                     [float(x) for x in np.linspace(optimalParams["halfToTime"] * 0.92,
        ...
        print('start data')
        statistics, totalMetric = _estimator(realTimeData.iloc[POSITION: POSITION + _TRADE_TIME].copy(),
...
if __name__ == '__main__':
    ...
    RES, EVOLUTION = _get_the_best_strategy(mainOptimizerData=inpData.copy().iloc[400_000:800_000],
